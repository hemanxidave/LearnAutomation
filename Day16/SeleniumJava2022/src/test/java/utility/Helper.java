...
public class Helper 
{
	...
	public static ArrayList<String> brokenImageChecker(List<WebElement> allImages) {
		...
		for(WebElement ele:allImages)
		{
				...
				try 
				{
					...
					if (code < 400) 
					{
						System.out.println("Image Displayed - Status Code "+ code  +" -URL " + url + " Status " + connection.getResponseMessage());
					} else 
					{
						System.out.println("Image Broken - Status Code "+ code  +"- URL " + url + " Status " + connection.getResponseMessage());
	...
	{
		WebElement element=new WebDriverWait(driver, Duration.ofSeconds(20)).until(ExpectedConditions.elementToBeClickable(locator));
	...
	{
		WebElement element=new WebDriverWait(driver, Duration.ofSeconds(time)).until(ExpectedConditions.elementToBeClickable(locator));
	...
	public static boolean enableWebElement(WebDriver driver,By locator)
	{
		...
		try 
		{
			status= (Boolean)js.executeScript("arguments[0].removeAttribute('disabled');",driver.findElement(locator));
		} catch (Exception e) {
